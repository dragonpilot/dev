...
inline int livestream_width() {
  switch (Hardware::get_device_type()) {
    // dp - tici shares tizi's 1928x1208 camera (camera.py maps ar0231/ox03c10 to the
    // same config for both), so it takes the same downscale. Upstream has no TICI case
    // only because 0.11.2 dropped tici from device_map entirely.
