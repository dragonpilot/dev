...
class TestTurnMiner(OpenpilotTestCase):

  def test_extract_signaled_right_turn(self):
    s = _stream([(6.0, 0.0, 8.0, "."), (3.14, 0.5, 4.0, "R"), (6.0, 0.0, 8.0, ".")])
    rows = extract_turns(s, "segX")
    assert rows
    assert sorted({r["maneuver_id"] for r in rows}) == [0]
  ...
  def test_extract_skips_unsignaled_turn(self):
  ...
  def test_extract_skips_maneuver_active_at_start(self):
  ...
  def test_extract_skips_maneuver_active_at_end(self):
  ...
  def test_extract_ignores_short_blinker_tap(self):
    s = _stream([(4.0, 0.0, 8.0, "."), (0.4, 0.5, 4.0, "R"), (4.0, 0.0, 8.0, ".")])  # 0.4s < MIN_TURN_DUR
    assert extract_turns(s, "segX") == []

  @unittest.skipIf(not os.path.isdir(_LOCAL_REALDATA), "no local realdata mount")
  def test_read_segment_returns_samples(self):
  ...
  @unittest.skipIf(not os.path.isdir(_LOCAL_REALDATA), "no local realdata mount")
  def test_mine_segment_writes_rows_if_any_signaled_turn(self, tmp_path):
    ...
    if not segs:
      self.skipTest("route not present")
  ...
  def test_read_segment_missing_file_returns_empty(self):
    assert read_segment("/no/such/rlog.zst") == []

  def test_report_computes_lead_times(self, tmp_path):
    import csv as _csv
    ...
    with open(p, "w", newline="") as f:
      ...
      for i, (y, d) in enumerate(zip(yaws, dtr, strict=True)):
