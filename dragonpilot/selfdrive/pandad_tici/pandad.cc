#include "dragonpilot/selfdrive/pandad_tici/pandad.h"
...
void send_peripheral_state(Panda *panda, PubMaster *pm) {
  ...
  // 0.11.2 dropped Hardware::get_voltage/get_current (hwmon) from the C++ hardware API;
  // read them from the panda health packet, as upstream pandad now does.
  auto health_opt = panda->get_state();
  if (health_opt) {
    health_t health = *health_opt;
    ps.setVoltage(health.voltage_pkt);
