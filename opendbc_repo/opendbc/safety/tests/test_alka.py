...
from opendbc.safety.tests.test_ford import checksum as ford_checksum
...
class TestALKAFord(TestALKABase):
  ...
  Ford is curvature-controlled, so its lateral gate is steer_curvature_cmd_checks,
  not steer_torque_cmd_checks. Constants mirror test_ford.py.
  """

  DEG_TO_CAN = 50000        # CAN units per rad/m
  LATERAL_FREQUENCY = 20    # Hz, for the per-frame jerk limit
  TEST_SPEED = 20.0         # m/s, above CURVATURE_ERROR_MIN_SPEED
  ...
  def _speed_msg(self, speed):
    ...
    return self.packer.make_can_msg_safety("BrakeSysFeatures", 0, values, fix_checksum=ford_checksum)

  # PCM vehicle speed (Ford checks two sources)
  def _speed_msg_2(self, speed):
    values = {"Veh_V_ActlEng": speed * 3.6, "VehVActlEng_D_Qf": 3, "VehVActlEng_No_Cnt": self.cnt_speed_2 % 16}
    self.__class__.cnt_speed_2 += 1
    return self.packer.make_can_msg_safety("EngVehicleSpThrottle2", 0, values, fix_checksum=ford_checksum)

  # Measured curvature
  def _yaw_rate_msg(self, curvature, speed):
    ...
    return self.packer.make_can_msg_safety("Yaw_Data_FD1", 0, values, fix_checksum=ford_checksum)

  def _set_vehicle_moving(self, moving: bool):
    # ford.h reads standstill from DesiredTorqBrk.VehStop_D_Stat (1 == stopped),
    # not from wheel speeds
    values = {"VehStop_D_Stat": 0 if moving else 1}
    for _ in range(6):
      self._rx(self.packer.make_can_msg_safety("DesiredTorqBrk", 0, values))
  ...
  def _set_prev_desired_curvature(self, curvature):
    self.safety.set_desired_curvature_last(round(curvature * self.DEG_TO_CAN))

  # LCA (curvature) command. Safety param 0 is the non-CAN-FD variant,
  # so LateralMotionControl is the permitted message.
  def _lat_ctl_msg(self, enabled: bool, curvature: float = 0.0):
  ...
  def _arm_alka(self):
    """Satisfy every ALKA condition with openpilot NOT engaged."""
    self._reset_safety_hooks()
    self.safety.set_alternative_experience(ALTERNATIVE_EXPERIENCE.ALKA)
    self._rx(self._acc_main_msg(1))
    self._set_vehicle_moving(True)
  ...
  def _torque_cmd_msg(self, torque, steer_req=1):
    raise NotImplementedError("Ford is curvature-controlled; use _lat_ctl_msg")

  def test_alka_allows_curvature_tx(self):
    """ALKA must grant curvature control exactly as being engaged does.

    Regression test for steer_curvature_cmd_checks gating on bare controls_allowed
    instead of lat_control_allowed(); Ford is the only alka_allowed brand using it.
    """
    # baseline: normally engaged
    self._reset_safety_hooks()
    self.safety.set_controls_allowed(True)
    self._set_prev_desired_curvature(0.0)
    self._reset_curvature_measurement(0.0, self.TEST_SPEED)
    self.assertTrue(self._tx(self._lat_ctl_msg(True)),
                    "engaged: curvature TX should be allowed (test precondition)")

    # ALKA armed, openpilot not engaged
    self._arm_alka()
    self.assertTrue(self.safety.get_alka_allowed())
    self.assertTrue(self.safety.get_lkas_on())
    ...
    self._reset_curvature_measurement(0.0, self.TEST_SPEED)
    self.assertTrue(self._tx(self._lat_ctl_msg(True)),
                    "ALKA armed: curvature TX should be allowed")

  def test_alka_curvature_blocked_when_stopped(self):
    """ALKA must not grant curvature control at standstill."""
    self._arm_alka()
    self._set_vehicle_moving(False)
  ...
  def test_alka_curvature_blocked_without_lkas_on(self):
    """ALKA must not grant curvature control when ACC main is off."""
    self._arm_alka()
    self._rx(self._acc_main_msg(0))
    self.assertFalse(self.safety.get_lkas_on())
    self._set_prev_desired_curvature(0.0)
  ...
  def test_alka_curvature_blocked_without_flag(self):
    """ALKA must not grant curvature control without ALT_EXP_ALKA."""
    self._arm_alka()
