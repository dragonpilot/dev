...
class CarState(CarStateBase):
  ...
  def update(self, can_parsers) -> structs.CarState:
    ...
    if self.zss.enabled:
      from opendbc.car.toyota.zss import ZssBus
      self.zss.set_values(can_parsers[ZssBus.zss])
    ...
    if (self.CP.flags & ToyotaFlags.TSS2 and not self.CP.flags & ToyotaFlags.DISABLE_RADAR.value) or \
      self.dp_dsu_bypass:
    ...
    if self.CP.flags & ToyotaFlags.TSS2:
      ...
      from opendbc.car.toyota.sdsu import SdsuBus
      self.sdsu.update_states(can_parsers[SdsuBus.sdsu])
      prev_distance_button = self.distance_button
  ...
  def get_can_parsers(CP):
    ...
    if CP.flags & ToyotaFlags.SDSU:
      from opendbc.car.toyota.sdsu import SdsuBus
      parsers[SdsuBus.sdsu] = CANParser("toyota_sdsu", [("SDSU", 100)], 0)
    if CP.flags & ToyotaFlags.ZSS:
      from opendbc.car.toyota.zss import ZssBus
      parsers[ZssBus.zss] = CANParser("toyota_zss", [("SECONDARY_STEER_ANGLE", float('nan'))], 0)
